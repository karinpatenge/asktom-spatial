------------------------------------------
-- Create random point data sets for WGS84
------------------------------------------
...
-- Insert 15 mio random point geometries
declare
    l_batch_num number := 15;
begin
    for i in 1 .. l_batch_num loop
        create_random_points(1000000);
    end loop;
end;
/
...
values (
    'POINTS_LONLAT',
    'SPATIALUSER.GET_GEOMETRY(longitude,latitude)',
    sdo_dim_array(
        sdo_dim_element('Lon',-180.0,180,0.05),
        sdo_dim_element('Lat',-90.0,900,0.05)
    ),
...
create index points_lonlat_sidx on points_lonlat(spatialuser.get_geometry(longitude,latitude))
indextype is mdsys.spatial_index_v2 parameters('layer_gtype=POINT cbtree_index=true')
parallel;
-- Takes ~ 10 mins for 15 mio lon/lat