...
values (1, 'Spatial data aggregation using SDO_AGGR_UNION', 1, 1, systimestamp);
...
where testcaseid = 1 and testrun = 1 and vpa_enabled = 1;
...
-- Begin test run 2
insert into vpa_test_results(testcaseid, testcasedesc, testrun, vpa_enabled, starttime)
values (2, 'Topological relationship using SDO_RELATE and masking operators TOUCH and COVEREDBY ', 1, 1, systimestamp);
...
update vpa_test_results
set endtime = systimestamp
where testcaseid = 2 and testrun = 1 and vpa_enabled = 1;
...
-- Begin test run 3
insert into vpa_test_results(testcaseid, testcasedesc, testrun, vpa_enabled, starttime)
values (3, 'Topological relationship using SDO_TOUCH', 1, 1, systimestamp);
...
update vpa_test_results
set endtime = systimestamp
where testcaseid = 3 and testrun = 1 and vpa_enabled = 1;
